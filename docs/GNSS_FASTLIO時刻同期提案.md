...
既存Ethernetでの試験機能は[PTP試験手順](../src/rtk_gps_um982/docs/PTP試験手順.md)を参照する。
GNSS受信フックからchronyへRMCを配信し、ソフトウェアPTPでPCのUTCをMID-360へ渡す。
有効時はGGA日付もRMCから補完する。生パケットの同期種別・epochを観測できる。
以下の課題表は調査時点の記録であり、日付補完とchrony配信についてはこの試験モードで対応済み。
ハードウェアPTPと位置/方位epoch結合、融合の自動停止は未実装である。
...
本書は設計提案であり、実装・試験操作の正は上記PTP試験手順とする。
実機の同期精度は別途測定する。
