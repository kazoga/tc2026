...
残り 30% は共通スロット化した補助表示エリアとし、左に最終出力 cmd、中央に状態固有の重要補助情報、
右に GNSS/RTK 状態を表示する。最終出力 cmd は全画面共通で常時表示し、並進速度は `m/s` 数値、
角速度は左旋回・右旋回・直進を矢印で示す。マニュアル走行と通常の自律走行では補助右スロットを
...
次 waypoint までの距離を表示する。自律復帰カウントダウン中のみ、復帰予定 cmd を補助右スロットの
優先表示にする。実装は `PyQt5` と `QGraphicsView` /
