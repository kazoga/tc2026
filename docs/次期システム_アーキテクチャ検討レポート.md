...
| 3.3 | 2026-05-28 | LLH初期実装では `route_msgs` を維持してfield追加し、`tc_route_msgs` 改名は次phaseに送る補足を追加した |
| 3.2 | 2026-05-28 | LLH対応に合わせ、現行 `route_msgs` を `tc_route_msgs` へ改名し、地理系interfaceを `tc_geo_msgs` とする方針を追加した |
...
| --- | --- | --- | --- |
| `route_msgs` | msg/srv 定義 | `Route`, `Waypoint`, `FollowerState`, `ObstacleAvoidanceHint` などの interface を提供する | LLH対応phaseで `tc_route_msgs` へ改名する。既存依存パッケージは同phaseで移行する |
...
- LLH対応phaseでは、汎用名の現行 `route_msgs` を `tc_route_msgs` へ改名し、新規地理系interfaceは `tc_geo_msgs` として接頭辞をそろえる。
- LLH 座標は、地図表示、ログ、外部連携、ルート編集時の入力形式として利用し、route msg内ではENU poseと同じwaypointに紐づけて保持する。
- LLH ベースで編集した route は、走行前にENU座標へ変換し、CSVと `/active_route` の両方でLLH情報を落とさず保持する。
