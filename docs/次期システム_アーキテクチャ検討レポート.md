...
QoS は topic ごとに個別指定せず、配信様式（`stream` / `latch` / `event`）から導出する。
規則は [トピック通信規約.md](トピック通信規約.md) 4 章を正とする。

各 topic の配信様式・公称レート・QoS の例外は、対応する `.msg` ファイルの先頭コメントに
宣言する。`.msg` を持たない topic は、配信元ノードの publisher 生成箇所に宣言する。

ノードの健全性判定に用いる topic の条件は
[ノード健全性監視設計.md](ノード健全性監視設計.md) 3 章を参照する。
