...
| `robot_navigator` | `robot_navigator`, `robot_simulator` | `/active_target` と自己位置から `cmd_vel` を生成する。簡易シミュレータも提供する | 継続使用する。実機では `/cmd_vel/autonomous` へ出力し、`drive_mode_manager` 経由で `ypspur_ros2` へ接続する |
...
- 実機運用では、`robot_navigator` の速度指令は `/cmd_vel/autonomous` に remap し、`drive_mode_manager` が最終 `/cmd_vel` を publish する。
...
速度指令については、実機運用では `robot_navigator` と `ypspur_node` を直接接続しない。
...
簡易シミュレーションでは、既存互換のため `robot_navigator -> robot_simulator` の直接接続を
残してもよい。ただし、手動介入や自律復帰を含む運用検証ではシミュレーションでも
`drive_mode_manager` を経由する構成を使う。
...
| Publish | `/drive_mode_status` | `route_msgs/msg/DriveModeStatus` | 走行状態、出力ソース、controller 状態、復帰カウントダウン |

`DriveModeStatus.msg` は、GUI・ログ解析・bringup 側の health 表示で共有される interface であるため、
実装時は `route_msgs` に追加する方針とする。既存 msg/srv の field は変更しない。
...
| `/drive_mode_status` | `route_msgs/msg/DriveModeStatus` | 自律・手動切替状態のサマリ表示。正本表示は専用 GUI |
...
| `/cmd_vel` | `geometry_msgs/msg/Twist` | 実機では `drive_cmd_mux_node`、簡易シミュレーションでは `robot_navigator` | `ypspur_ros2`, simulator, `robot_console` |
...
| `/drive_mode_status` | `route_msgs/msg/DriveModeStatus` | `drive_cmd_mux_node` | `drive_status_gui_node`, `robot_console` optional, debug tools |
...
| `robot_navigator` の `cmd_vel_topic` | 実機で `/cmd_vel` へ直接出すか、mux 入力へ分離するか | 実機では `/cmd_vel/autonomous` へ remap し、`drive_cmd_mux_node` が最終 `/cmd_vel` を publish する |
...
| `DriveModeStatus.msg` 配置 | `route_msgs` に追加するか、`drive_mode_manager` 側に interface を持たせるか | 共有 interface として `route_msgs` へ追加する |
...
publish する。実機運用では `robot_navigator` の出力を `/cmd_vel/autonomous` に remap し、
PS3 controller 由来の `/cmd_vel/manual` とともに `drive_mode_manager` へ入力する。
