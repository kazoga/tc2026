...
- シミュレーション構成
- 今後追加すべき bringup、変換、融合、LiDAR adapter パッケージ
...
| --- | --- | --- | --- |
| `route_msgs` | msg/srv 定義 | `Route`, `Waypoint`, `FollowerState`, `ObstacleAvoidanceHint` などの interface を提供する | 継続使用する |
| `route_planner` | `route_planner` | YAML/CSV から `Route` を生成し、ルート取得・更新 service を提供する | 継続使用する。走行時 route は `map` frame の ENU 座標を正本とする |
| `route_manager` | `route_manager`, `active_route_marker` | ルート取得、アクティブルート配信、再計画判断、状態配信を行う | 継続使用する |
| `route_follower` | `route_follower`, `active_target_marker` | `/active_route` と自己位置から `/active_target` を生成する | 継続使用する。自己位置入力は `/amcl_pose` 互換 topic を維持する |
| `robot_navigator` | `robot_navigator`, `robot_simulator` | `/active_target` と自己位置から `cmd_vel` を生成する。簡易シミュレータも提供する | 継続使用する。実機では `cmd_vel` を `ypspur_ros2` へ接続する |
...
| `robot_console` | `robot_console` | 走行状態、経路、速度、画像、手動操作の GUI を提供する | 継続使用し、OSM 表示と GNSS/RTK 状態表示を追加する |
| `yolo_detector` | `yolo_node`, `yolo_ncnn_node`, `road_blockage_detector` など | 画像認識、道路封鎖検知、カメラ模擬を行う | 継続使用する。ただし次期では YOLO 推論層へ責務を限定し、道路封鎖判定は別パッケージへ分離する |
...
`robot_navigator` が `cmd_vel` を publish する構造である。
...
- 走行時に使う `Route`、Waypoint、自己位置、制御目標は `map` frame の local ENU 座標で統一する。
- `Route` の正本は ENU 座標とする。
- LLH 座標は、地図表示、ログ、外部連携、ルート編集時の入力形式として利用する。
- LLH ベースで編集した route は、走行前に ENU 座標へ変換した CSV として保存する。
...
- `geo_pose_converter` は LLH/ENU 変換、原点管理、heading 規約管理、OSM 表示用 LLH topic 生成を担当する。
- `localization_fusion` は GNSS 由来 ENU pose、LiDAR odometry、必要に応じて wheel odometry を融合し、`/amcl_pose` 互換 topic を publish する。
- `route_follower`, `robot_navigator`, `obstacle_monitor` は、当面 `/amcl_pose` と `/odom` という既存 topic 名を維持して接続する。
- 実ロボットの速度指令先は `ypspur_ros2` とする。
- `robot_navigator` と `ypspur_ros2` を同一 namespace なしで起動する場合、`robot_navigator` の `cmd_vel` はそのまま `ypspur_node` の `/cmd_vel` に接続できる。
...
| データ | frame/topic | 方針 |
| --- | --- | --- |
| `Route.header.frame_id` | `map` | local ENU |
| Waypoint 位置 | `x`, `y`, `z` | ENU 座標 |
| Waypoint 姿勢 | quaternion | ENU yaw から生成 |
| 自己位置 | `/amcl_pose` | `map` frame の `PoseWithCovarianceStamped` |
| active target | `/active_target` | `map` frame の `PoseStamped` |
| 速度指令 | `/cmd_vel` | `base_link` 基準の `Twist` |
...
flowchart LR
  ...
  LOC -- "/amcl_pose<br/>PoseWithCovarianceStamped" --> FOLLOWER
  LOC -- "/amcl_pose<br/>PoseWithCovarianceStamped" --> NAV
  ...
  MANAGER -- "/active_route<br/>Route in map/ENU" --> FOLLOWER
  MANAGER -- "/active_route<br/>Route in map/ENU" --> PROJECTOR
  ...
  NAV -- "/cmd_vel<br/>Twist" --> YPSPUR
  NAV -- "/direction_marker" --> RVIZ

  PROJECTOR -- "/localization/pose_llh" --> GUI
  PROJECTOR -- "/active_route_llh" --> GUI
  PROJECTOR -- "/active_target_llh" --> GUI

  CAM --> YOLO_RB
  CAM --> YOLO_SIG
  YOLO_RB -- "/perception/road_blockage/detections" --> ROAD_DET
  YOLO_RB -- "/perception/road_blockage/image_det" --> GUI
  ...
  YOLO_SIG -- "/perception/traffic_signal/image_det" --> SIG_REC
  FOLLOWER -- "/recog_flag" --> YOLO_SIG
  FOLLOWER -- "/recog_flag" --> SIG_REC
  SIG_REC -- "/sig_recog" --> FOLLOWER
  SIG_REC -- "/sig_recog<br/>/sig_det_imgs" --> GUI
...
LLH 系 topic を `route_geo_projector_node` から `robot_console` へ入力する形で表す。
OSM 表示用の座標変換そのものは `robot_console` 内に実装せず、`geo_pose_converter` /
`route_geo_projector_node` が publish する `/localization/pose_llh`,
`/active_route_llh`, `/active_target_llh` を購読して表示する。
...
`/amcl_pose` は名前としては AMCL 由来ではないが、既存ノード互換のため当面維持する。
将来、`/localization/pose_enu` へ移行する場合も、互換 alias の廃止時期を明確にしてから行う。
...
flowchart TD
  ...
  RN["robot_navigator"]
  OM["obstacle_monitor"]
  LOC["localization_fusion<br/>/amcl_pose"]
...
速度指令については、どちらも namespace なしで起動する場合、`robot_navigator` の
`/cmd_vel` と `ypspur_node` の `/cmd_vel` が直接接続される。namespace を分ける場合は、
`robot_navigator` の `cmd_vel_topic` launch 引数で `ypspur_node` 側へ remap する。
...
flowchart LR
  ...
  YOLO_SIG -- "/perception/traffic_signal/image_det" --> SIGNAL
  SIGNAL -- "/sig_recog" --> FOLLOWER
  SIGNAL -- "/sig_recog" --> GUI
  SIGNAL -- "/sig_det_imgs" --> GUI
```
...
| 出力 | `/road_blocked` | `/sig_recog`, `/sig_det_imgs` |
...
`/sig_det_imgs` による可視化という機能要件を継承する。
...
- `/active_route` と `/active_target` を LLH 表示用 topic に変換する。
- local ENU 原点、原点高度、heading 変換規約、高度の扱いを一元管理する。
- LLH waypoint CSV から ENU 走行用 CSV を生成する service または tool を提供する。
...
| Publish | `/localization/pose_llh` | 独自 LLH pose msg | OSM 表示用自己位置 |
| Subscribe | `/active_route` | `route_msgs/msg/Route` | ENU route |
| Publish | `/active_route_llh` | 独自 LLH route msg | OSM 表示用 route |
| Subscribe | `/active_target` | `geometry_msgs/msg/PoseStamped` | ENU active target |
| Publish | `/active_target_llh` | 独自 LLH pose msg | OSM 表示用 active target |
...
| Publish | `/amcl_pose` | `geometry_msgs/msg/PoseWithCovarianceStamped` | 既存ノード互換の alias |
...
- `rtk_gps_um982`, Livox driver, Fast LIO2, `geo_pose_converter`,
  `localization_fusion`, route stack, `ypspur_ros2` の統合起動
- 実機用 topic remap の一元管理
- `robot_navigator` の `odom_topic` と `cmd_vel_topic` を `ypspur_ros2` に合わせる
...
| --- | --- | --- |
| `yolo_detector_road_blockage` | 経路封鎖看板モデルの推論 | `/perception/road_blockage/detections`, `/perception/road_blockage/image_det` |
| `yolo_detector_traffic_signal` | 信号 red/green モデルの推論 | `/perception/traffic_signal/detections`, `/perception/traffic_signal/image_det` |

#### 7.5.2 `road_blockage_detector`

`road_blockage_detector` は、YOLO 検出結果から経路封鎖イベントを判定する新規または分離先
パッケージである。現状 `yolo_detector` パッケージ内にある `road_blockage_detector_node.py`
の責務を、最終的にはこのパッケージへ移す。
...
├── docs/
└── tests/
    └── test_road_blockage_core.py
```

主な責務は以下である。

- `/perception/road_blockage/detections` を購読する。
- `/amcl_pose` を購読し、検出時の自己位置を参照する。
...
- 必要に応じて `/perception/traffic_signal/image_det` または `/usb_cam/image_raw` を購読し、
  信号監視用画像を `/sig_det_imgs` として publish する。
...
| `/sig_det_imgs` | `sensor_msgs/msg/Image` | Publish | 信号認識結果を重畳した監視画像 |
...
| `/localization/pose_llh` | 独自 LLH pose msg | OSM 上の自己位置 |
| `/active_target_llh` | 独自 LLH pose msg | OSM 上の active target |
| `/active_route_llh` | 独自 LLH route msg | OSM 上の route |

LLH 変換は GUI 内で都度 service 呼び出しするのではなく、`route_geo_projector_node` が
変換済み topic を publish し、`robot_console` は購読に徹する構成を推奨する。
...
この構成では `rtk_gps_um982` と `ypspur_ros2` は起動しない。route stack の回帰確認を
主目的とする。
...
`geo_pose_converter` 以降を実機と同じ接続で検証する。

### 10.3 Gazebo 障害物回避・ルート復帰検証構成

Gazebo は、GNSS/LiDAR 融合の検証ではなく、障害物回避とルート復帰挙動の確認に使う。

この構成では、Gazebo または bridge node が `/odom`, `/amcl_pose`, `/scan` を提供し、
route stack は既存 topic 名で接続する。
...
| `/rtk_gps/rtk_status` | `rtk_gps_um982_msgs/msg/RtkStatus` | `rtk_gps_um982` | `geo_pose_converter`, `robot_console` |
| `/cmd_vel` | `geometry_msgs/msg/Twist` | `robot_navigator` | `ypspur_ros2` |
...
| `/localization/pose_llh` | 独自 LLH pose msg | `route_geo_projector` | `robot_console` |
| `/active_route_llh` | 独自 LLH route msg | `route_geo_projector` | `robot_console` |
| `/active_target_llh` | 独自 LLH pose msg | `route_geo_projector` | `robot_console` |
...
| 種別 | 名前 | Type | 継続理由 |
| --- | --- | --- | --- |
| Topic | `/active_route` | `route_msgs/msg/Route` | route stack の中核。ENU 正本 |
| Topic | `/active_target` | `geometry_msgs/msg/PoseStamped` | 速度指令生成の目標入力 |
| Topic | `/amcl_pose` | `geometry_msgs/msg/PoseWithCovarianceStamped` | 既存ノード互換の自己位置入力 |
| Topic | `/scan` | `sensor_msgs/msg/LaserScan` | 既存 `obstacle_monitor` 入力 |
| Topic | `/obstacle_avoidance_hint` | `route_msgs/msg/ObstacleAvoidanceHint` | 回避・減速判断 |
| Service | `/get_route` | `route_msgs/srv/GetRoute` | 初期 route 取得 |
| Service | `/update_route` | `route_msgs/srv/UpdateRoute` | 再計画 |
| Service | `/report_stuck` | `route_msgs/srv/ReportStuck` | follower から manager への滞留通知 |
...
| `/perception/road_blockage/image_det` | `sensor_msgs/msg/Image` | `yolo_detector_road_blockage` | `robot_console`, debug tools | 経路封鎖検知の重畳画像 |
...
| `/perception/traffic_signal/image_det` | `sensor_msgs/msg/Image` | `yolo_detector_traffic_signal` | `traffic_signal_recognizer`, debug tools | 信号検出矩形の重畳画像 |
| `/sig_recog` | `std_msgs/msg/Int32` | `traffic_signal_recognizer`, `robot_console` | `route_follower`, `robot_console` | `1=GO`, `2=STOP/NOGO` |
| `/sig_det_imgs` | `sensor_msgs/msg/Image` | `traffic_signal_recognizer` | `robot_console` | 信号監視用画像。tc2025 互換 topic |
...
| `/active_route_llh` | `RELIABLE`, `TRANSIENT_LOCAL`, `KEEP_LAST(1)` |
| `/amcl_pose` | `RELIABLE`, `KEEP_LAST(10)` |
...
| `/perception/*/image_det`, `/sig_det_imgs` | 画像表示用途のため `BEST_EFFORT`, `KEEP_LAST(1)` を基本とする |
...
- `robot_navigator` の `cmd_vel_topic` と `odom_topic` を `ypspur_ros2` の実 topic 名に合わせる。
...
- 現行 `yolo_detector` 内の `road_blockage_detector_node.py` を `road_blockage_detector` パッケージへ分離する。
...
- 信号認識では、旧 ROS 1 実装の 3 回連続 green 判定、信頼度閾値、`/sig_det_imgs` 可視化を機能として継承する。
- `route_follower`, `robot_console` との互換性維持のため、`/recog_flag`, `/sig_recog`, `/sig_det_imgs` の topic 名は当面維持する。
...
- LLH waypoint CSV から ENU 走行用 CSV を生成する tool または service を整備する。

### Phase 4: Localization fusion の追加

- `localization_fusion` が `/gnss/pose_enu`, `/lio/odometry`, 必要に応じて wheel odometry を購読する。
- `/localization/pose_enu` と `/amcl_pose` を publish する。
- `route_follower`, `robot_navigator`, `obstacle_monitor` は当面 `/amcl_pose` 互換で接続する。
- TF 発行責務を確定する。

### Phase 5: OSM 可視化と RTK 状態表示

- `robot_console` に OSM 表示を追加する。
- `/localization/pose_llh`, `/active_target_llh`, `/active_route_llh` を表示する。
...
| `/sig_det_imgs` 生成元 | `yolo_detector` の重畳画像を直接 GUI 表示するか、信号判定結果を重ね直すか | `traffic_signal_recognizer` が GO/STOP 判定も重畳して `/sig_det_imgs` を publish する |
| ENU 原点 | コース固定原点か、起動時初回 GNSS か | 再現性のためコース固定原点を推奨する |
| 高度 | 楕円体高、ジオイド高、固定 z の扱い | 初期は走行 z は route CSV 優先、LLH 表示では GNSS 高度保持を推奨する |
...
publish する。`robot_navigator` は既存の `cmd_vel` 出力を維持したまま接続できるが、
`odom_topic` の既定値と `ypspur_ros2` の実 topic 名には差があるため、bringup で明示的に
揃える必要がある。

既存 route stack は、`/amcl_pose`, `/odom`, `/active_route`, `/active_target`,
`/cmd_vel` の互換 interface を保つことで継続利用できる。したがって、次期開発の中心は
`geo_pose_converter`, `localization_fusion`, `mid360_pointcloud_adapter`,
`tc2026_system_bringup` の追加と、topic/remap/TF 責務の整理である。
