...
通行幅用の蓄積はGNSSの絶対補正を使わない。地図表示は従来どおり融合位置へ配置する。
...
基準候補の上限 `surface_max_seed_intensity=30` は今回のMID360/路面用で、
...
停止理由を表示する。元の保存ルートは上書きせず、再記録時から新方式を使う。

反射強度の閾値は環境普遍ではない。edc24727の感度確認では、平坦な芝生を路面15に近い強度20にした合成入力で境界を見逃した。低い芝・濡れ・別舗装への適用精度は未検証であり、幾何と反射強度だけで芝生を確実に識別するものではない。
