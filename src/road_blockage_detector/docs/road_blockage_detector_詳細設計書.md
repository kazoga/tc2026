...
- 封鎖確定後、`route_follower` 側で `stagnation_duration_sec` が経過すると滞留と判定され、`report_stuck` を `route_manager` に要求する。`route_manager` は固定区間では現在経路を再配信し、可変区間ではplannerの更新を試す。
  経路更新や封鎖の回避が必ず成功するわけではない。
...
| warn | `/localization/pose_enu` 未取得時、Detection と `/localization/pose_enu` の時刻差が 3 秒以上ある場合、検知メッセージが連続で欠損した場合。 |
