...
**実装状態:** 本書が定義する `ConsoleCore`、PyQt5 UI、HTML UIは実装済みであり、正式entry pointは PyQt5 版（`robot_console_qt`）と HTML遠隔観測UI（`robot_console_web`）である。旧 `tkinter` 版（`robot_console`）は実装・設計書とも削除済みである。
