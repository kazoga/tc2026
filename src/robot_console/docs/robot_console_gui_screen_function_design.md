...
パネルは1列に縦積みし、上から `front_camera`、`sensor_viewer` の順に並べる。左右へ
並べると1枚あたりの幅が列幅の半分になり、横長画像では縦方向が余って表示が小さくなる。
この並びはQt UIとHTML UIで共通とする。HTML UIでは配信順に任せず、カメラを先頭に
固定する（どちらのtopicが先に届いたかで上下が入れ替わらないようにする）。
HTML UIのカード幅には上限を設ける。セクション全幅まで広げると、PCでは4:3映像の高さが
1画面に収まらなくなるため。上限によりPC幅ではセクション内に余りが出るので、カードは
中央へ寄せる（画像だけを中央にすると、見出し行が全幅へ広がり画像と離れる）。

カメラ行とその他の行の高さ比は3:2とする。等分にするとカメラ行だけが判定チップの高さを
負担し、画像領域がSensor Viewerより小さくなる。これ以上カメラ側へ寄せると、16:9映像では
枠が縦に余りSensor Viewerが縮むだけになる。

パネル名とtopic・鮮度・最終更新時刻は同じ見出し行へ左右振り分けで置く。見出しと状態行を
別々の行にすると2行分の縦を消費し、その分だけ画像領域が狭くなる。

地図とセンサ列の横比（3:2）は変更しない。縦積みでは高さが律速であり、センサ列を
広げてもカメラ表示は大きくならず、左右の余白が増えるだけであるため。
...
チップはQt UI・HTML UIともカメラパネルのカード内（画像の直下）へ置く。画面全幅の別カード
にすると、どの画像に対する判定なのかが視覚的に結び付かないうえ、見出しと枠の分だけ縦を
消費してカメラ画像が小さくなる。種別名（信号 / 経路封鎖）をチップ自身が持つため、
カード見出しは設けない。カメラパネルが構成に無い場合もチップの枠は残す。
