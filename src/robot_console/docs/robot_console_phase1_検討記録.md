...
5. **画像パネル（3面固定表示）**
   ...
   - 外部カメラ領域は信号停止 waypoint 待機中は `/perception/traffic_signal/decision_image` を表示し、それ以外は `/perception/road_blockage/decision_image` を表示する。いずれも受信できない場合は「No Image」プレースホルダを描画する。
