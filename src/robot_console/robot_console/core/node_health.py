...
def apply_diagnostics(
    ...
    診断が届かないこと自体は異常としない。外部パッケージは配信しないため、
    不在と異常を区別できないからである。
...
def select_diagnostics(
    ...
    return [entry for entry in diagnostics.values() if entry.node_name in targets]
...
def build_summary(
    *,
    ...
    topic_health = aggregate_topic_freshness(
        topic_levels, within_startup_grace=within_startup_grace
    )
    if not topic_levels:
        # health_topics を持たない profile は子プロセス状態だけで判定する。
...
def _launch_status_only(launch_status: NodeLaunchStatus) -> tuple:
    """health_topics を持たない profile の表示状態。"""
