...
### 2.3 関連トピックとの関係
- `obstacle_monitor` は `/scan` から前方帯域を解析し、閉塞フラグとともに距離ヒント `front_clearance_m` を `ObstacleAvoidanceHint` で publish している。【F:ros2_src/obstacle_monitor/obstacle_monitor/obstacle_monitor_node.py†L192-L217】【F:ros2_src/tc_route_msgs/msg/ObstacleAvoidanceHint.msg†L1-L8】
...
- `package.xml` に `tc_route_msgs` の依存を追加し、`ObstacleAvoidanceHint` 型を import できるようにする。【F:ros2_src/robot_navigator/package.xml†L1-L19】
