...
def prepare_host(output: Path, user: str) -> None:
    ...
    fragment.write_text(fragment.read_text().replace('offset 0.0', 'offset 0.0 prefer'))
...
chronyc tracking
printf '設定完了。退避先: %s\\nGNSSセンサを起動し、UM98選択と収束を確認してからPTP試験を開始してください。\\n' "$backup_dir"
''')
    script.chmod(0o755)
    (output/'README.txt').write_text(
        '既存USB + Ethernet配線。GNSS RMC → chrony → ptp4l -S → MID360。\n'
        ...
        'PTPは自動起動しない。GNSS選択・収束を確認してptp_trial runを使う。\n'
