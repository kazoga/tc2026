...
1. `std_msgs/Int32` 型 Publisher `recog_flag` を新設する。QoS は他トピックと同様の RELIABLE/Volatile を使用し、bridge のデフォルト動作に合わせる。
...
6. 起動直後の `recog_flag=0` を取りこぼさないよう、IDLE→RUNNING 遷移時にも 0 を明示的に再送して橋渡しを確実にする。
7. トピック名は ROS1 側と揃えて `recog_flag` を維持し、bridge 側はデフォルトの動的ブリッジ（`ros1_bridge`）で `std_msgs/Int32` を透過させる。
...
- RouteFollowerNode に `recog_flag` Publisher を追加し、WAITING_STOP で `signal_stop=True` のウェイポイント待機中のみ `1` を送る。それ以外の状態では `0` を維持し、IDLE→RUNNING 遷移時には起動直後の 0 を改めて送信してブリッジ越しの欠落を防ぐ。【F:ros2_src/route_follower/route_follower/route_follower_node.py†L133-L206】【F:ros2_src/route_follower/route_follower/route_follower_node.py†L303-L451】
